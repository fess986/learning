...
class Visitor { // абстрактный класс который регламентирует вид наследников
  createVisit() {
    throw new Error(`Вы забыли создать метод createVisit в классе ${this.constructor.name}`)
  }
}

class RenderVisitor extends Visitor { // посетитель который тупо рендерит исходный объект через его же метод render
  createVisit(resipt,data) {
    return resipt.render(data);
  }
}

class TitleChangingVisitor extends Visitor {  // посетитель, который меняет входные данные
  createVisit(resipt,data) {
    data.title = data.title + 'ass'  // дописываем в титл сообщение. Можно делать например по условию
    // console.log(data)
  }
}

class Recipt {  // класс, к которому мы будем приклеивать визетера
  accept(visitor, data) {  // метод, который позволяет активировать посетителей
    return visitor.createVisit(this, data)
  }
...
const renderSection = (data, nameSection) => {
  const getRecipesBlock = () => {
    ...
    const renderVisitor = new RenderVisitor()
    const titleChangingVisitor = new TitleChangingVisitor()

    data.forEach(function (element) {
      const resipt = new Recipt()
      // booksBlock += renderRecipes(element); //стандартный рендеринг
      // booksBlock += new Recipt().render(element) //  рендеринг через класс
      // booksBlock += resipt.render(element) //  рендеринг через класс
      resipt.accept(titleChangingVisitor, element);
      booksBlock += resipt.accept(renderVisitor, element)  // рендерим через визитёра
    });
    booksBlock += `</ul>`;
    return booksBlock;
  };
