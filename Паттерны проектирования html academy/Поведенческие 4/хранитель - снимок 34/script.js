...
// наш хранитель, который умеет в себе сохранять данные и достовать их в случае необходимости
class Memento {
  state = 'ass'  // поле в котором мы храним данные

  GetState() {  // геттер
    return this.state;
  }

  SetState(state) {  // сеттер
    this.state = state
  }
}

const saveButton = document.getElementById('save');
const restreButton = document.querySelector('#restore')

const saver = new Memento(); // создаем инстанс

saveButton.addEventListener('click', () => {
  const promptText = prompt('сохраним значение')
  saver.SetState(promptText) // сохраняем значение
  console.log(saver.GetState())
})

restreButton.addEventListener('click', () => {
  alert(saver.GetState());  // воспроизводим сохраненное значение
})
