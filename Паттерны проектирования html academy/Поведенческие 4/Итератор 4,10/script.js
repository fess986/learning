...
class Iterator {  // запускает действия с объектом data, если выполняется условия condition
  ...
  next() {
    if (!this.condition) {
      for (let field in SECTION) {
        const sectionBooks = this.data.filter((book) => book.type === field);
...
const condition = currentTime > 10 && currentTime < 12;
// const condition = false;

getDataFromServer(`${window.location.origin}/data.json`, function (err, data) {

  const iterator = new Iterator(data, condition);
  iterator.next();
