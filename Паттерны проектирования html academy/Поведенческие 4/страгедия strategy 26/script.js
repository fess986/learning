...
const renderRecipesWithoutAuthor = (data) => {
  return `
    <li class='recipes__item'>
      <div class='recipes__inner'><img class='recipes__img' src='/static/images/content/${data.img}' alt='${data.title}'>
      <div class='recipes__box'>
...
// так записывать видимо нельзя, так как стрелочная функция не имеет контекста. Но вообще это хрень, почему нельзя было просто использовать классы?
// const RenderStrategy = (strategy) => {
// this.strategy = strategy;
// }

const RenderStrategy = function (strategy) {
  this.strategy = strategy;
};

// обязательно задаем функционально
RenderStrategy.prototype.render = function (data) {
  return this.strategy(data);
};

// мой вариант с классами, хз зачем они делали псевдоклассы
class RenderStrategy2 {
...
const renderSection = (data, nameSection) => {
  const getRecipesBlock = () => {
    ...
    data.forEach(function (element) {
      // далее мы по условию, ну или просто так передаем в качестве конструктора-рендера 2 версии
      booksBlock += new RenderStrategy(renderRecipesWithoutAuthor).render(element);
      booksBlock += new RenderStrategy2(renderRecipesWithoutAuthor).render(element);
    });
    booksBlock += `</ul>`;
    return booksBlock;
  };
