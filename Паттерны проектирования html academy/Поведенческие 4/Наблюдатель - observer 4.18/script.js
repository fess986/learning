...
class Mediator { // класс с реализацией дополнительного функционала, в данном случае добавление стилей для рамки
  addBorder() {
...
class Recipes {
  constructor(mediator) {  // принимаем в себя объект с реализованным методом
    this.mediator = mediator;
  }

  createBorder() {  // используем реализацию другого класса
    return this.mediator.addBorder(); 
  }

  render(data) {
    return `
      <li class='recipes__item' style='${this.createBorder()}'>
...
const renderSection = (data, nameSection) => {
  const getRecipesBlock = () => {
    let recipesBlock = `<ul class="recipes">`;
    data.forEach(function (element) {
      recipesBlock += new Recipes(new Mediator()).render(element);  // используем класс и передаем в него медиатор
    });
    recipesBlock += `</ul>`;
    return recipesBlock;
  };
