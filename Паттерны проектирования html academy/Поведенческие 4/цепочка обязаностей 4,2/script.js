...
class Recipt {
  ...
  areAvailable(id) {  // возращаем объект типа {ID, COUNT, title = NAME}
    let result = null;

    for (let places in LOCAL_BASE) {
      if (LOCAL_BASE[places].NAME === this.type) {  // смотрим только в текущем ресторане-объекте 
        LOCAL_BASE[places].PRODUCTS.map((product) => {
          if (product.ID === id) {
            result = product;
            result.placeName = LOCAL_BASE[places].NAME; // дописываем в найденный продукт - имя ресурса, где он был найден
          }
        })
      }
    }
  ...
  check(id) { // проверяем есть ли в этом объекте нужный товар по id, возвращаем титл в виде "3 продукта в ресторан Изабель"

    let product = this.areAvailable(id);
    let titleText = '';

    if (product?.COUNT > 0 ) {
      return titleText = `В ${product.placeName} находится ${product.COUNT} единиц товара`
    } else if (this.nextObject) {
      // console.log('123')
      return this.nextObject.check(id)
    } else {
      return 'нет товара'
    }
...
const checkingAvalibility = (id) => { // создаем функцию, которая проверяет наличие товаров в указанных магазинах
  const rapuncel = new Rapuncel('кафе Рапунсель');
  const isabel = new Isabel('ресторан Изабель');
  rapuncel.setNext(isabel)

  return rapuncel.check(id)
}
    ...
    <li class='recipes__item'>
      ...
      <div class='recipes__box'>
        ...
        <p class='recipes__description'>${data.discr}</p>
          <span class='recipes__ingredients'>${checkingAvalibility(data.id)} </span>
