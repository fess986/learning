...
const changeThemeSetter = () => {
  ...
  // храним состояние приложения внутри функции или объекта/класса . 
  const state = {
    current : 'old',
  }

  // в зависимости от состояния объекта/класса мы делаем нужные нам вещи
  const changeTheme = () => {
    state.current === 'old' 
    ? (console.log('yes'), state.current = 'new', button.innerHTML = 'Сменить на синий цвет', button.className = 'new')                                            
    : (console.log('no'), state.current = 'old', button.innerHTML = 'Сменить на красный цвет', button.className = 'old')
  }                     

  // навешиваем в качестве коллбека нашу функцию
  button.onclick = () => {
    changeTheme()
  }
}

changeThemeSetter();  // запускаем нашу функцию