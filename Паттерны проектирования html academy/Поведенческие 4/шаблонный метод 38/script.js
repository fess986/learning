...
// создаем шаблонный метод со специальными методами
class TemplateResipt {
  title = null  // тут будет храниться добавляемый текст к титлу
  book_title = null  // итоговый титл с добавкой

  addToTitle(data) { // метод для склейки титлов
    this.book_title = this.title + data.title
  }

  isNew() {  // по умолчанию рецепт не имеет пометни "новый"
    return false
  }

  isBest() {// по умолчанию рецепт не имеет пометни "лучший"
    return false
  }

  render(data) {  // обычный рендер, с нюансом - в титл записываем или новый склеенный титл или же обычный, если первый - null
    return `
    <li class='recipes__item'>
      ...
      <div class='recipes__box'>
        ...
        <h3 class='recipes__name'>${this.book_title ? this.book_title : data.title}</h3>
  ...
  renderWithCondition(data) { // меняем по необходимости титл и рендерим
    if (this.isNew() || this.isBest()) {
...
class  NewResipt extends TemplateResipt {  // "новый рецепт"
  title = 'new';
...
class  BestResipt extends TemplateResipt {  // "Лучший рецепт"
  title = 'best';
...
const renderSection = (data, nameSection) => {
  const getRecipesBlock = () => {
    ...
    data.forEach(function (element) {
      if (nameSection === 'new') {  // рендерим в соответствии с условием или новый или лучший рецепт
        booksBlock += new NewResipt().renderWithCondition(element);
      } else if (nameSection === 'best') {
        booksBlock += new BestResipt().renderWithCondition(element);
      }
